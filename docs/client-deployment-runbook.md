...
npm run legacy:cutover -- \
  ...
  --customer-recovery-manifest /protected/recovery/2026-07-31-customer-recovery-approval-v4.json \
  --payment-date-policy invoice-date-proxy \
  --dry-run --report --summary-only
```

4. Generate the deterministic Customer-and-Vehicle proposal, review every Customer and Vehicle candidate, and create the explicitly approved private Recovery Approval v4. A proposal is not authorization, and an exact VIN match is not automatic authorization. The proposal, approval, both candidate-set hashes, source fingerprint, file hashes, snapshot manifest, ZIP, and shop binding must all match this accepted snapshot; a new ZIP requires a new proposal and approval. Historical v1/v2 and Customer-only v3 artifacts cannot authorize strengthened final cutover.
