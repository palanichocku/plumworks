...
`legacy:rehearse` is a dry-run-only orchestration command. It accepts either the read-only repository seed or a customer-supplied ZIP, creates an immutable snapshot, reconstructs and validates the exact Customer-and-Vehicle recovery proposal and approved Recovery Approval v4, captures shop-scoped counts, runs the consolidated cutover dry run, validates its ordered stage ledger and reconciliation results, captures the same counts again, and runs the focused legacy checks.

Version 1, version 2, and Customer-only v3 recovery files are compatibility artifacts and cannot authorize strengthened final-cutover rehearsal. Generate a proposal, review every Customer and Vehicle candidate, and create explicit Recovery Approval v4 using `docs/legacy-recovery-manifest-upgrade.md`.
...
npm run legacy:rehearse -- \
  ...
  --customer-recovery-manifest /protected/path/recovery-approval-v4.json \
...
npm run legacy:rehearse -- \
  ...
  --customer-recovery-manifest /protected/path/2026-07-31-recovery-approval-v4.json \
...
The JSON and Markdown reports are sanitized aggregate reports. They record Git state, ZIP identity, source fingerprint, proposal and Recovery Approval v4 Customer/Vehicle fingerprints, ordered stage validation, Customer/Vehicle recovery linkage, Invoice/AR, Payment and open-order aggregates, before/after database counts, tests, lint, warnings, and any failed stage. Invoice-date payment groupings remain explicitly labeled as proxy allocations rather than receipt chronology.
...
Fresh normal legacy transforms do not infer archive state. Normally imported Customers and Vehicles are created active (`archivedAt = null`) because no authoritative legacy archive field has been established. A v4-created recovered historical Vehicle is different: it is archived by explicit reviewed recovery semantics so it contributes to historical lookup but cannot be selected for a new Repair Order. A reviewer may explicitly approve active state only when the evidence supports operational use.

Before the accepted final cutoff, a rehearsal is repeatable and Windows remains authoritative. An explicitly authorized `parallel-baseline` replacement is likewise non-terminal and records its own audit event; it requires a dedicated confirmation and authority-through date. A `final-production` cutover records the terminal event, makes Windows read-only, and permanently blocks every later Windows full replacement. Historical `legacy_cutover_completed` events remain backward-compatible default-mode blockers. Archive/delete decisions are not part of recovery manifest v2.
