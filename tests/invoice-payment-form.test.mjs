...
const [invoicePage, paymentForm, paymentAction] = await Promise.all([
  read("src/app/(app)/invoices/[id]/page.tsx"),
  read("src/components/invoice-payment-form.tsx"),
...
test("payment amount defaults to the authoritative remaining balance with two decimals", () => {
  ...
  assert.match(paymentForm, /name="amount"[^>]*max=\{remainingBalance\}[^>]*step="0\.01"[^>]*defaultValue=\{remainingBalance\}/);
...
test("amount remains an editable uncontrolled input across unrelated rerenders", () => {
  assert.match(paymentForm, /name="amount" type="number"/);
  assert.doesNotMatch(paymentForm, /name="amount"[^>]*(?:readOnly|disabled|value=)/);
  assert.doesNotMatch(paymentForm, /useEffect|setAmount|onChange=\{/);
});
...
test("fully paid invoices do not offer zero-dollar payment entry", () => {
  ...
  assert.match(paymentForm, /min="0\.01"/);
});
...
test("server-side positive amount, precision, and overpayment validation remain intact", () => {
  ...
  assert.match(paymentAction, /applyInvoicePayment\(invoice\.total, existingPaid, amount\)/);
});
