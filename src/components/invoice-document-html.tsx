...
export function InvoiceDocumentHTML({ model }: { model: InvoiceDocumentModel }) {
  ...
  return <article className="invoice-document">
    ...
    <section className="invoice-document-meta">
      ...
      <div><h2>Vehicle</h2>{model.vehicle ? <><p>{[model.vehicle.year, model.vehicle.make, model.vehicle.model].filter(Boolean).join(" ") || "Vehicle details unavailable"}</p><dl><Detail label="Engine" value={model.vehicle.engine} /><Detail label="VIN" value={model.vehicle.vin} /><Detail label="License" value={model.vehicle.licensePlate} /><Detail label="Miles out" value={model.vehicle.odometer?.toLocaleString()} /></dl></> : <p>Vehicle not linked</p>}</div>
