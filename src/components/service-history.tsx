...
type InvoiceHistoryItem = { id: string; legacyRoNo: string | null; repairOrderNumber: number | null; invoiceDate: Date | null; total: { toString(): string }; parts: HistoryPart[]; vehicle?: RelatedVehicle; customer?: RelatedCustomer };
...
type DisplayEntry = { source: RepairOrderHistorySource; id: string; legacyRoNo: string | null; repairOrderNumber: number | null; date: Date | null; total: { toString(): string }; parts: HistoryPart[]; vehicle?: RelatedVehicle; customer?: RelatedCustomer };
        ...
        {entries.map((entry) => {
          ...
          return <li key={`${entry.source}:${entry.id}`} className="px-6 py-4 transition hover:bg-slate-50">
            <button type="button" onClick={(event) => { triggerRef.current = event.currentTarget; setSelected({ source: entry.source, id: entry.id }); }} className="grid w-full gap-2 rounded text-left focus:outline-none focus:ring-4 focus:ring-brand-primary/10 sm:grid-cols-[1fr_1fr_auto] sm:items-center" aria-label={`Open internal service record RO ${entry.repairOrderNumber ?? entry.legacyRoNo ?? "Not recorded"}`}>
              <span><span className="block font-semibold text-brand-primary underline-offset-2 hover:underline">RO #{entry.repairOrderNumber ?? entry.legacyRoNo ?? "Not recorded"}</span><span className="text-sm text-slate-500">{formatDate(entry.date)}</span>{entry.parts.length > 0 ? <span className="mt-3 block space-y-2">{entry.parts.map((part) => <span key={part.id} className="block text-sm"><span className="block font-medium text-slate-800">{part.description}</span><span className="block text-xs text-slate-500">Part #: {part.partNumber ?? "Not recorded"} · Vendor: {part.vendorNameSnapshot?.trim() || "Not recorded"}</span></span>)}</span> : null}</span>
