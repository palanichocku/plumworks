...
export function LeadReadControl({ notification }: { notification: LeadReadNotification | null }) {
  const { state, pendingReadIds, viewLead } = useLeadNotifications();
  ...
  return <div className="mb-4 flex items-center justify-between gap-3 rounded-lg bg-orange-50 px-3 py-2 text-sm text-orange-800">
    ...
    <button disabled={pendingReadIds.has(notification.id)} onClick={async () => { if (await viewLead(notification, false)) setReadHere(true); }} className="font-semibold underline disabled:opacity-50">Mark as read</button>
  </div>;
}
