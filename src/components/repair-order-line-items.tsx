...
import { addLineItemButtonClass, CheckIcon, ClearLineItemButton, deleteLineItemButtonClass, LineItemAmountActions, PendingIcon, PlusIcon, saveLineItemButtonClass, TrashIcon } from "@/components/line-item-layout";
...
function DraftPartRow({ repairOrderId, vendors, onReset }: { repairOrderId: string; vendors: VendorOption[]; onReset: () => void }) {
  ...
  return <PartActionForm action={addPartLineWithState} onSuccess={onReset} className="space-y-3 rounded-lg border border-slate-300 bg-slate-50/70 p-3">
    <input type="hidden" name="repairOrderId" value={repairOrderId} /><HistoricalDescriptionCombobox kind="part" rowKey="draft-part" value={description} onChange={setDescription} inputClass={descriptionClass} placeholder="Part description" multiline /><div className="ro-part-controls grid min-w-0 items-end gap-3"><VendorCombobox vendors={vendors} /><label className="text-sm font-semibold text-slate-700">Quantity<input name="quantity" type="number" required min="0.01" max="1000000" step="0.01" value={quantity} onChange={(event) => setQuantity(event.target.value)} className={inputClass} /></label><label className="text-sm font-semibold text-slate-700">Unit price<input name="unitPrice" type="number" required min="0" max="1000000" step="0.01" value={unitPrice} onChange={(event) => setUnitPrice(event.target.value)} className={inputClass} /></label><LineItemAmountActions amount={Number(quantity) * Number(unitPrice)}><FormSubmitButton pendingLabel={<PendingIcon />} pendingAriaLabel="Adding part" ariaLabel="Add part" title="Add part" className={addLineItemButtonClass}><PlusIcon /></FormSubmitButton><ClearLineItemButton label="Clear part" onClear={onReset} /></LineItemAmountActions></div>
  </PartActionForm>;
}
...
function LaborActionForm({ action, children, onSuccess }: { action: (state: LaborActionState, formData: FormData) => Promise<LaborActionState>; children: React.ReactNode; onSuccess?: () => void }) {
  ...
  return <form action={formAction} className="space-y-3 rounded-lg border border-slate-300 p-3">{children}<div aria-live="polite">{state.status === "error" && <p role="alert" className="rounded-lg bg-red-50 px-3 py-2 text-sm font-medium text-red-700">{state.message}</p>}{state.status === "success" && <p className="text-sm font-medium text-emerald-700">Saved.</p>}</div></form>;
}
...
function SavedLaborRow({ repairOrderId, line, services }: { repairOrderId: string; line: LaborLine; services: CommonService[] }) {
  ...
  const selectService = (service: CommonService) => { setDescription(service.description); setHours(service.defaultHours); setRate(service.defaultLaborRate); setEligible(service.shopSuppliesEligible); };
  return <LaborActionForm action={updateLaborLineWithState} onSuccess={() => setSaved({ ...line, description, hours, hourlyRate: rate, shopSuppliesEligible: eligible })}><input type="hidden" name="repairOrderId" value={repairOrderId} /><input type="hidden" name="laborLineId" value={line.id} /><input type="hidden" name="shopSuppliesEligible" value={String(eligible)} /><LaborDescription value={description} onChange={setDescription} rowKey={line.id} /><div className="ro-labor-controls grid min-w-0 items-end gap-3"><label className="text-sm font-semibold text-slate-700">Hours<input name="hours" type="number" required min="0.01" max="1000" step="0.01" value={hours} onChange={(event) => setHours(event.target.value)} className={inputClass} /></label><label className="text-sm font-semibold text-slate-700">Rate<input name="hourlyRate" type="number" required min="0" max="1000000" step="0.01" value={rate} onChange={(event) => setRate(event.target.value)} className={inputClass} /></label><label className="flex min-h-10 items-center gap-2 text-sm font-semibold text-slate-700"><input type="checkbox" checked={eligible} onChange={(event) => setEligible(event.target.checked)} className="h-4 w-4 shrink-0 rounded border-slate-300 text-brand-primary" />Apply Shop Supplies</label><LineItemAmountActions amount={Number(hours) * Number(rate)}><FormSubmitButton disabled={!dirty} pendingLabel={<PendingIcon />} pendingAriaLabel="Saving labor" ariaLabel="Save labor" title="Save labor" className={saveLineItemButtonClass}><CheckIcon /></FormSubmitButton><SavedDeleteButton action={deleteLaborLine} label="Delete labor" /></LineItemAmountActions></div><CommonServiceSelect services={services} onSelect={selectService} /></LaborActionForm>;
}
...
function DraftLaborRow({ repairOrderId, services, defaultRate, onReset }: { repairOrderId: string; services: CommonService[]; defaultRate: string; onReset: () => void }) {
  ...
  const selectService = (service: CommonService) => { setDescription(service.description); setHours(service.defaultHours); setRate(service.defaultLaborRate); setEligible(service.shopSuppliesEligible); };
  return <LaborActionForm action={addLaborLineWithState} onSuccess={onReset}><input type="hidden" name="repairOrderId" value={repairOrderId} /><input type="hidden" name="shopSuppliesEligible" value={String(eligible)} /><LaborDescription value={description} onChange={setDescription} rowKey="draft-labor" /><div className="ro-labor-controls grid min-w-0 items-end gap-3"><label className="text-sm font-semibold text-slate-700">Hours<input name="hours" type="number" required min="0.01" max="1000" step="0.01" value={hours} onChange={(event) => setHours(event.target.value)} className={inputClass} /></label><label className="text-sm font-semibold text-slate-700">Rate<input name="hourlyRate" type="number" required min="0" max="1000000" step="0.01" value={rate} onChange={(event) => setRate(event.target.value)} className={inputClass} /></label><label className="flex min-h-10 items-center gap-2 text-sm font-semibold text-slate-700"><input type="checkbox" checked={eligible} onChange={(event) => setEligible(event.target.checked)} className="h-4 w-4 shrink-0 rounded border-slate-300 text-brand-primary" />Apply Shop Supplies</label><LineItemAmountActions amount={Number(hours) * Number(rate)}><FormSubmitButton pendingLabel={<PendingIcon />} pendingAriaLabel="Adding labor" ariaLabel="Add labor" title="Add labor" className={addLineItemButtonClass}><PlusIcon /></FormSubmitButton><ClearLineItemButton label="Clear labor" onClear={onReset} /></LineItemAmountActions></div><CommonServiceSelect services={services} onSelect={selectService} /></LaborActionForm>;
}
...
function CommonServiceSelect({ services, onSelect }: { services: CommonService[]; onSelect: (service: CommonService) => void }) {
  ...
  return <label className="block w-full max-w-2xl text-xs font-semibold text-slate-600">Common Service<select aria-label="Common Services" defaultValue="" onChange={(event) => { const service = services.find((item) => item.id === event.target.value); if (service) onSelect(service); event.target.value = ""; }} className="mt-1 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 focus:border-brand-primary focus:outline-none focus:ring-4 focus:ring-brand-primary/20"><option value="">Choose a Common Service…</option>{services.map((service) => <option key={service.id} value={service.id}>{service.name}</option>)}</select></label>;
}
