...
export function FormSubmitButton({ children, pendingLabel = "Saving…", pendingAriaLabel, confirmMessage, confirmTitle, confirmDescription, confirmLabel, cancelLabel, destructive = false, className, disabled = false, formAction, form, title, ariaLabel }: { children: React.ReactNode; pendingLabel?: React.ReactNode; pendingAriaLabel?: string; confirmMessage?: string; confirmTitle?: string; confirmDescription?: string; confirmLabel?: string; cancelLabel?: string; destructive?: boolean; className: string; disabled?: boolean; formAction?: (formData: FormData) => void | Promise<void>; form?: string; title?: string; ariaLabel?: string }) {
  ...
  return <><button ref={buttonRef} type="submit" form={form} formAction={formAction} disabled={disabled || pending} title={title} aria-label={pending ? pendingAriaLabel ?? ariaLabel : ariaLabel} onClick={(event) => { if (!needsConfirmation) return; event.preventDefault(); const targetForm = event.currentTarget.form; if (targetForm && !targetForm.reportValidity()) return; setOpen(true); }} className={className}>{pending ? pendingLabel : children}</button>{needsConfirmation && <ConfirmDialog open={open} title={confirmTitle ?? legacyTitle ?? "Confirm action"} description={confirmDescription ?? legacyDescription ?? (inferredDestructive ? "This cannot be undone." : "Are you sure you want to continue?")} confirmLabel={confirmLabel} cancelLabel={cancelLabel} destructive={inferredDestructive} pending={pending} onConfirm={confirm} onCancel={close} />}</>;
}
