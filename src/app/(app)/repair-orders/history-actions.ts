...
export async function loadRepairOrderHistory(customerId: string, vehicleId?: string, currentRepairOrderId?: string, cursor?: RepairOrderHistoryCursor | null) {
  try {
    const result = await getCustomerRepairOrderHistory(customerId, vehicleId, currentRepairOrderId, cursor);
...
export async function loadRepairOrderHistoryDetail(customerId: string, vehicleId: string | undefined, currentRepairOrderId: string | undefined, source: unknown, historicalId: string) {
  try {
    const detail = await getCustomerRepairOrderHistoryDetail(customerId, vehicleId, currentRepairOrderId, source, historicalId);
