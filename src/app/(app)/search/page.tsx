...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  const { terminology } = getBusinessProfile();
  ...
  return <>
    <PageHeading eyebrow="Shop search" title="Search" description={`Find customers, ${terminology.assetPlural.toLowerCase()}, ${terminology.workOrderPlural.toLowerCase()}, and invoices in your current shop.`} />
    <form action="/search" className="mb-7 flex gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><label htmlFor="global-search-page" className="sr-only">Search shop</label><input id="global-search-page" name="q" type="search" defaultValue={query} autoFocus placeholder={`Name, phone, ${terminology.assetSingular.toLowerCase()}, VIN, license, or ${terminology.workOrderAbbreviation} number`} className="min-w-0 flex-1 rounded-lg border border-slate-300 px-3 py-2.5 text-sm" /><button type="submit" className="rounded-lg bg-brand-primary px-4 py-2.5 text-sm font-semibold text-white">Search</button></form>
