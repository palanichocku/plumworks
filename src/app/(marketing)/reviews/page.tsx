...
export default async function ReviewsPage() {
  ...
  return <>
    ...
    {testimonials.length ? <section className="mx-auto max-w-5xl px-4 py-16 sm:px-6">
      ...
      <div className="grid gap-5 md:grid-cols-3">{testimonials.map((item) => <blockquote key={item.id} className="rounded-2xl border border-slate-200 bg-white p-6"><div aria-label={`${item.rating ?? 5} out of 5 stars`} className="text-orange-500">{"★".repeat(item.rating ?? 5)}</div><p className="mt-4 font-bold">“{item.quote}”</p>{item.attribution ? <footer className="mt-4 text-xs text-slate-500">{item.attribution}</footer> : null}</blockquote>)}</div>
