import { serviceRequestCopy } from "@/lib/marketing-requested-services";
import type { Metadata } from "next";
import Link from "next/link";
import { MarketingPageHero } from "@/components/marketing/page-hero";
import { getMarketingServices } from "@/lib/marketing-content";
import { getPublicSeoShop, localTitle, marketingMetadata } from "@/lib/marketing-seo";

export async function generateMetadata(): Promise<Metadata> { const shop = await getPublicSeoShop(); return marketingMetadata({ title: localTitle("Auto Repair Services", shop), description: `Explore maintenance, diagnostics, brake repair, oil service, electrical work and other vehicle services available from ${shop.name} in ${shop.city ?? "the local area"}.`, path: "/services", siteName: shop.name }); }
export default async function ServicesPage() { const services = await getMarketingServices(); return <><MarketingPageHero eyebrow="Services" title="Practical care for everyday driving" description={serviceRequestCopy} /><section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8"><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{services.map((service) => <Link key={service.slug} href={`/services/${service.slug}`} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:border-orange-300"><p className="text-xs font-black uppercase tracking-widest text-orange-600">Auto care</p><h2 className="mt-3 text-xl font-black">{service.name}</h2><p className="mt-3 text-sm leading-6 text-slate-600">{service.summary}</p><p className="mt-5 text-sm font-bold text-orange-600">Learn more →</p></Link>)}</div></section></>; }
