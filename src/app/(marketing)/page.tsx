...
export default async function MarketingHome() {
  ...
  return <>
      ...
      <div className="mx-auto grid max-w-7xl lg:min-h-[620px] lg:grid-cols-[1.05fr_.95fr]">
        <div className="flex items-center px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
          <div className="max-w-2xl">
            ...
            <p className="mt-6 max-w-xl text-lg leading-8 text-slate-300">{shop.name} offers vehicle maintenance, diagnostics, and repair. Call to discuss your vehicle, or start an online service request.</p>
    ...
    {featuredServices.length ? <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
      <div className="max-w-3xl"><p className="text-sm font-black uppercase tracking-widest text-orange-700">Core services</p><h2 className="mt-3 text-3xl font-black tracking-tight sm:text-4xl">Help for common vehicle needs</h2><p className="mt-4 text-lg leading-8 text-slate-600">{serviceRequestCopy}</p></div>
    ...
    <section className="border-y border-slate-200 bg-stone-100"><div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
      ...
      <ol className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">{[["1", "Start a conversation", "Select a service category and preferred contact method. The shop will contact you to get the details."], ["2", "Evaluate the vehicle", `${shop.name} inspects or tests the vehicle as appropriate to the concern.`], ["3", "Review the findings", "The shop explains what it found in understandable terms."], ["4", "Discuss the options", "Review the practical recommendation, priorities, and estimate."], ["5", "Approve the work", "Repair work proceeds after you decide what to authorize."]].map(([number, title, body]) => <li key={number} className="rounded-2xl bg-white p-5 shadow-sm"><span className="flex size-9 items-center justify-center rounded-full bg-orange-100 font-black text-orange-800">{number}</span><h3 className="mt-5 text-lg font-black">{title}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{body}</p></li>)}</ol>
    ...
    <section className="border-t border-slate-200 bg-white"><div className="mx-auto grid max-w-7xl gap-8 px-4 py-16 sm:px-6 sm:py-20 lg:grid-cols-[1fr_.8fr] lg:px-8">
      <div><p className="text-sm font-black uppercase tracking-widest text-orange-700">Location and contact</p><h2 className="mt-3 text-3xl font-black tracking-tight sm:text-4xl">Talk with {shop.name}</h2><p className="mt-4 max-w-xl leading-7 text-slate-600">Call for the quickest conversation, or select a service category and preferred contact method online. The shop will contact you to get the details.</p><div className="mt-7 flex flex-wrap gap-3">{shop.phone ? <TrackedCallLink href={phoneHref(shop.phone)} className={`rounded-xl bg-slate-950 px-5 py-3 font-black text-white hover:bg-slate-800 ${focusRing}`}>Call {shop.phone}</TrackedCallLink> : null}{directionsUrl ? <a href={directionsUrl} target="_blank" rel="noreferrer" className={`rounded-xl border border-slate-300 bg-white px-5 py-3 font-black text-slate-800 hover:bg-slate-50 ${focusRing}`}>Get Directions</a> : null}<AttributionLink href="/appointment" className={`rounded-xl bg-orange-600 px-5 py-3 font-black text-white hover:bg-orange-700 ${focusRing}`}>Request Service</AttributionLink></div></div>
