...
function calculateNativeTransactionTotals(input: TransactionTotalsInput) {
  ...
  return { partsTotal, laborTotal, subtotal, shopSuppliesAmount, shopSuppliesEligibleLaborTotal, shopSuppliesCalculatedAmount: shopSuppliesAmount, taxTotal, total: subtotal.plus(shopSuppliesAmount).plus(taxTotal).toDecimalPlaces(2) };
}
